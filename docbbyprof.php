...
<link rel="stylesheet" href="docbbyprof.css">
</head>
<body>
<div class="container">
...
<script>
  ...
  function addCustomVaccine(name,count){var tbody=document.getElementById('immunizationTableBody');var groupCount=tbody.querySelectorAll('.vac-col').length;var shade=(groupCount%2===0)?'pink':'purple';for(var i=1;i<=count;i++){var tr=document.createElement('tr');if(i===1){var tdType=document.createElement('td');tdType.textContent=name;tdType.rowSpan=count;tdType.className='vac-col '+shade;tr.appendChild(tdType)}var tdDose=document.createElement('td');tdDose.textContent=i;tr.appendChild(tdDose);var tdLoc=document.createElement('td');tdLoc.innerHTML='<input type="text" placeholder="site/clinic">';var tdDate=document.createElement('td');tdDate.innerHTML='<input type="date">';var tdReact=document.createElement('td');tdReact.innerHTML='<input type="text" placeholder="reaction">';var tdRemarks=document.createElement('td');tdRemarks.innerHTML='<input type="text" placeholder="remarks">';tr.appendChild(tdLoc);tr.appendChild(tdDate);tr.appendChild(tdReact);tr.appendChild(tdRemarks);tbody.appendChild(tr)}}
  function buildImmunizationTable(){var tbody=document.getElementById('immunizationTableBody');tbody.innerHTML='';Object.keys(vaccineDoseMap).forEach(function(type,idx){var count=vaccineDoseMap[type];for(var i=1;i<=count;i++){var tr=document.createElement('tr');if(i===1){var tdType=document.createElement('td');tdType.textContent=type;tdType.rowSpan=count;tdType.className='vac-col '+((idx%2===0)?'pink':'purple');tr.appendChild(tdType)}var tdDose=document.createElement('td');tdDose.textContent=i;tr.appendChild(tdDose);var tdLoc=document.createElement('td');tdLoc.innerHTML='<input type="text" placeholder="site/clinic">';var tdDate=document.createElement('td');tdDate.innerHTML='<input type="date">';var tdReact=document.createElement('td');tdReact.innerHTML='<input type="text" placeholder="reaction">';var tdRemarks=document.createElement('td');tdRemarks.innerHTML='<input type="text" placeholder="remarks">';tr.appendChild(tdLoc);tr.appendChild(tdDate);tr.appendChild(tdReact);tr.appendChild(tdRemarks);tbody.appendChild(tr)}})}
  function applySavedImmunization(){var tbody=document.getElementById('immunizationTableBody');var saved=getSavedImmunization();if(!saved.length)return;var map={};saved.forEach(function(r){var key=(r.type||'')+'|'+(r.dose||'');map[key]=r});var currentType='';[].slice.call(tbody.rows).forEach(function(r){var cells=[].slice.call(r.cells);var type, dose; if(cells.length===6){type=cells[0].textContent.trim();dose=cells[1].textContent.trim()} else {dose=cells[0].textContent.trim();type=currentType} currentType=type||currentType;var key=(currentType||'')+'|'+dose;var data=map[key];function setCell(cell,val){if(!val)return;cell.innerHTML='';var span=document.createElement('span');span.className='saved-value';span.textContent=val;cell.appendChild(span)}; if(data){var idxShift=(cells.length===6?2:1);setCell(cells[idxShift],data.location);setCell(cells[idxShift+1],data.date);setCell(cells[idxShift+2],data.reaction);setCell(cells[idxShift+3],data.remarks)}})}
  var customModal=document.getElementById('customVaccineModal');
    ...
    document.getElementById('confirmAddCustomVaccine').addEventListener('click',function(){var name=document.getElementById('customVaccineName').value.trim();var count=parseInt(document.getElementById('customVaccineDose').value,10)||0;if(!name||count<1){customModal.classList.add('hidden');return}addCustomVaccine(name,count);customModal.classList.add('hidden');document.getElementById('customVaccineName').value='';document.getElementById('customVaccineDose').value='1'});
    document.getElementById('saveImmunization').addEventListener('click',function(){var rows=[];var tbody=document.getElementById('immunizationTableBody');var currentType='';[].slice.call(tbody.rows).forEach(function(r){var cells=[].slice.call(r.cells);function getCellValue(cell){var inp=cell.querySelector('input');if(inp){return inp.value}else{return cell.textContent.trim()}}
      if(cells.length===6){currentType=cells[0].textContent.trim();rows.push({type:currentType,dose:cells[1].textContent.trim(),location:getCellValue(cells[2]),date:getCellValue(cells[3]),reaction:getCellValue(cells[4]),remarks:getCellValue(cells[5])})}
      else if(cells.length===5){rows.push({type:currentType,dose:cells[0].textContent.trim(),location:getCellValue(cells[1]),date:getCellValue(cells[2]),reaction:getCellValue(cells[3]),remarks:getCellValue(cells[4])})}
  });localStorage.setItem('immunization',JSON.stringify(rows));
  tbody.querySelectorAll('input').forEach(function(inp){var span=document.createElement('span');span.className='saved-value';span.textContent=inp.value;inp.parentNode.innerHTML='';inp.parentNode.appendChild(span)});
});
...
window.addEventListener('load',function(){buildImmunizationTable();applySavedImmunization();setVisitDateMin()});
